...
interface TimelineEvent {
  ...
  year: string;
  location?: string;
  type: 'debut' | 'performance' | 'achievement' | 'milestone' | 'mv' | 'special';
  image: string;
  category: string;
}
...
const IdolTimeline: React.FC = () => {
  const events: TimelineEvent[] = [
    ...
    {
      id: 8,
      title: "Renai Kinshi Jourei Shonichi",
      description: "Tampil memukau dalam pertunjukan Shonichi setlist Renai Kinshi Jourei. Performance yang menunjukkan growth dan development sebagai performer yang semakin matang.",
      date: "10 Oktober",
      year: "2024",
      location: "JKT48 Theater",
      type: "performance",
    ...
    {
      id: 10,
      title: "Sukinanda Meet & Greet",
      description: "Melanjutkan interaksi dengan fans melalui acara Sukinanda. Sesi meet & greet yang penuh kehangatan dan apresiasi timbal balik antara idol dan fans.",
      date: "11 Februari",
      year: "2025",
      location: "Jakarta",
      type: "milestone",
    ...
    },
    {
      ...
      description: "Melaksanakan pertunjukan Shonichi dengan setlist Te wo Tsunaginagara. Performance terbaru yang menampilkan kedewasaan dan skill yang terus berkembang sebagai idol profesional.",
      date: "24 Mei",
      year: "2025",
      location: "JKT48 Theater",
      type: "performance",
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-white rounded-xl shadow-sm border border-pink-100">
            <div className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-rose-500 bg-clip-text text-transparent">12</div>
            <div className="text-sm text-gray-600">Total Events</div>
          </div>
          <div className="text-center p-4 bg-white rounded-xl shadow-sm border border-pink-100">
            <div className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-rose-500 bg-clip-text text-transparent">4</div>
            <div className="text-sm text-gray-600">Performances</div>
          </div>
          <div className="text-center p-4 bg-white rounded-xl shadow-sm border border-pink-100">
            <div className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-rose-500 bg-clip-text text-transparent">2</div>
            <div className="text-sm text-gray-600">Music Videos</div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-16">
        <div className="relative">
          ...
          {events.map((event, index) => (
            <div key={event.id} className="relative mb-16 group">
              {/* Year Badge */}
              <div className={`absolute ${index % 2 === 0 ? 'left-16 md:left-1/2 md:ml-8' : 'left-16 md:right-1/2 md:mr-8'} -top-4 z-20`}>
                <div className="bg-gradient-to-r from-pink-500 to-rose-400 text-white px-6 py-1 rounded-full text-sm font-semibold shadow-lg border-4 border-pink-400 w-32 text-center mx-auto">
                  {event.year}
                </div>
              </div>
              ...
              <div className="absolute left-3 md:left-1/2 md:transform md:-translate-x-1/2 w-6 h-6 z-20 mt-8">
                <div className={`w-full h-full rounded-full bg-gradient-to-r ${getEventGradient(event.type)} shadow-lg flex items-center justify-center text-white transform transition-all duration-300 group-hover:scale-125 group-hover:shadow-xl`}>
              ...
              <div className={`flex ${index % 2 === 0 ? 'justify-start md:justify-end' : 'justify-start'} w-full`}>
                <div className={`w-full md:w-5/12 ml-12 md:ml-0 ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'}`}>
                  <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group-hover:transform group-hover:-translate-y-2 border border-pink-100">
                    {/* Image */}
                    <div className="relative overflow-hidden">
                      ...
                      {/* Remove badge for Sousenkyo Rank 18 Achievement */}
                      {event.title !== 'Sousenkyo Rank 18 Achievement' && (
                    ...
                    <div className="p-6">
                      ...
                      {event.location && (
                        <div className="flex items-center text-pink-500 text-sm mb-3">
        ...
        <div className="relative flex justify-center mt-12">
          <div className="w-8 h-8 rounded-full bg-gradient-to-r from-pink-400 via-rose-300 to-white shadow-xl border-4 border-white relative z-10">
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-pink-400 to-rose-300 opacity-20 animate-ping"></div>
          </div>
        </div>
        </div>
      </div>
